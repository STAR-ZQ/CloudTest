...
public class ExcelExportService extends BaseExportService {
    ...
    private int createHeaderRow(ExportParams title, Sheet sheet, Workbook workbook, int index,
        ...
        row.setHeight((short) (title.getHeaderHeight() * 50));
        Row listRow = null;
        if (rows >= 2) {
        ...
        CellStyle titleStyle = ExcelExportUtil.exportStyle(workbook, 280, HorizontalAlignment.CENTER);
    ...
    public void createSheet(Workbook workbook, ExportParams entity, Class<?> pojoClass,
                            Collection<?> dataSet,Map<Object,Object> ignoreData) {
        ...
        try {
            ...
            //获取所有表格列参数（根据注解获取对应属性值创建表格实体对象）
            getAllExcelField(entity.getExclusions(), targetId, fileds, excelParams, pojoClass,
                    null, null,ignoreData);
            //获取所有参数后,后面的逻辑判断就一致了
...
//            sortAllParams(excelParams);
            ...
            while (its.hasNext()) {
                Object t = its.next();
                index += createCells(patriarch, index, t, excelParams, sheet, workbook, rowHeight, 0, isBound)[0];
                tempList.add(t);
                if (index >= MAX_NUM) {
                    break;
                }
            ...
            sheet.addMergedRegion(new CellRangeAddress(index, index, 0, 17));
            row.setHeight((short) 450);
            Cell cell = row.createCell(0);
            cell.setCellValue("合计:条数" + subTotal + ",入仓重量");
            cell.setCellStyle(ExcelExportUtil.exportStyle(workbook, 300, HorizontalAlignment.LEFT));
