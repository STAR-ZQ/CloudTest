...
public abstract class BaseExportService extends ExportCommonService {
    ...
    public int[] createCells(Drawing patriarch, int index, Object t,
                             ...
                             short rowHeight, int cellNum, String isBound) {
        try {
            ...
            for (int k = indexKey, paramSize = excelParams.size(); k < paramSize; k++) {
                ...
                if (entity.getList() != null) {
                    ...
                    if (list != null && list.size() > 0) {
                        int tempCellNum = 0;
                        for (Object obj : list) {
                            int[] temp = createCells(patriarch, index + tmpListHeight, obj, entity.getList(), sheet, workbook, rowHeight, cellNum, isBound);
                            tempCellNum = temp[1];
                        ...
                        cellNum = tempCellNum;
                        //进/出仓单导出计算小计
                        sheet.addMergedRegion(new CellRangeAddress(index + tmpListHeight, index + tmpListHeight, 0, 17));
                        //创建小计行
                        row = sheet.createRow(index + tmpListHeight);
                        tmpListHeight++;

                        row = sheet.getRow(index + tmpListHeight - 1);
                        Cell cell = row.createCell(0);
                        cell.setCellValue("测试小计行");
                        cell.setCellStyle(ExcelExportUtil.exportStyle(workbook, 260, HorizontalAlignment.LEFT));
