 ...
 */
package com.zq.stream.test.utils;
...
public class ExportCommonService {
    ...
    private ExcelExportEntity createExcelExportEntity(Field field, String targetId,
                                                      Class<?> pojoClass,
                                                      List<Method> getMethods, ExcelEntity excelGroup, Map<Object, Object> ignoreData) throws Exception {
        Excel excel = field.getAnnotation(Excel.class);
        ExcelExportEntity excelEntity = new ExcelExportEntity();
        excelEntity.setType(excel.type());
        //为实体对象赋值（从注解取值）
        getExcelField(targetId, field, excelEntity, excel, pojoClass, excelGroup, ignoreData);
        if (getMethods != null) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (field.getAnnotation(Excel.class) != null) {
                ...
                if (StringUtils.isNotBlank(name)) {
                    //创建导出实体对象（导出创建对象的主要操作）
                    excelParams.add(createExcelExportEntity(field, targetId, pojoClass, getMethods, excelGroup, ignoreData));
                }
                ...
                getAllExcelField(exclusions,
                        ...
                        PoiPublicUtil.getClassFields(clz), list, clz, null, null, ignoreData);
                ...
                getAllExcelField(exclusions,
                        ...
                        newMethods, excel.show() ? excel : null, ignoreData);
            }
        }
    }
    ...
    /**
     * 注解到导出对象的转换（自定义列头需修改指定赋值：例如：新增列头参数，然后判断excel注解定义好的name是否存在自定义列头中）
     */
    private void getExcelField(String targetId, Field field, ExcelExportEntity excelEntity,
                               Excel excel, Class<?> pojoClass, ExcelEntity excelGroup, Map<Object, Object> ignoreData) throws Exception {
        excelEntity.setName(PoiPublicUtil.getValueByTargetId(excel.name(), targetId, null));
        ...
        //如果自定义列对象为空的话则按照注解导出，不为空按照自定义列导出
        if (MapUtils.isNotEmpty(ignoreData)) {
            //如果该列存在于自定义列集合中  导出就展示 否则隐藏
            if (ignoreData.containsKey(excel.name())) {
                excelEntity.setColumnHidden(false);
            } else {
                excelEntity.setColumnHidden(true);
            }
        } else {
            //控制属性列导出是否展示
            excelEntity.setColumnHidden(excel.isColumnHidden());
        }
        //如果主表自定义列头，明细列列头正常导出，则判断该实体类是否为该明系列对象，如果是则按照注解导出，不做其他操作
        //        if (pojoClass.getName().equals(Class.class.getName())){
        //            excelEntity.setColumnHidden(excel.isColumnHidden());
        //        }
