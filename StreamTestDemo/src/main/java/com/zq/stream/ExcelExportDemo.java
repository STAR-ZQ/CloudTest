...
//        cell.setCellValue("合计:");
        ...
        row1.createCell(0).setCellValue("合计：订单总额:"+sum);
...
//        XSSFCell cell3 =row3.createCell(13);
        spreadsheet.addMergedRegion(new CellRangeAddress(list.size()+1, list.size()+1, 0, 14));
//        spreadsheet.addMergedRegion(new CellRangeAddress(list.size()+3, list.size()+3, 12, 14));

        FileOutputStream out = new FileOutputStream(new File("testExport.xlsx"));
        workbook.write(out);
        out.close();
        System.out.println("testExport.xlsx written successfully");
    }
